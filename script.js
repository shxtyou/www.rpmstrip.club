...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function updateFinalPrice(forcedName = null) {
    const selected = forcedName || serviceSelect.value;
    const all = [...services.briz, ...services.lisa, ...services.both];
    const promo = promoInput?.value?.trim() || '';
    const s = all.find(x => x.name === selected) || { price: 150000 };
    let price = s.price;
  ...
  // VIP кнопка
  document.getElementById('vipOrderBtn').addEventListener('click', () => {
    openOrderPopup();
    personSelect.disabled = true;
    serviceSelect.innerHTML = '<option selected>VIP-подписка</option>';
    serviceSelect.disabled = true;
    document.getElementById('orderDate').style.display = 'none';
    document.getElementById('promoCode').style.display = 'none';
    updateFinalPrice('VIP-подписка');
  });
